...
class BurnMyWindowsWispsEffect {
  constructor() {
    ...
    this.shader = effect.addFragmentShader(Effect.MapTexture, 'wisps.frag');
  ...
  slotWindowAdded(window) {
    ...
    if (!BurnMyWindowsWispsEffect.shouldAnimateWindow(window)) {
      return;
    }
    if (!window.visible) {
      return;
